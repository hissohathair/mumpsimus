...
void
ulog(int priority, const char *message, ...)
{
  va_list args;
  ...
  if (getenv(ULOG_ENVVAR) != NULL)
    {
      ...
      if (*endptr)
	fprintf(stderr,
		"%s: ERROR -- %s must be a number between 1 and 7 (default=%d; got '%s')\n",
		ident, ULOG_ENVVAR, default_log_level, getenv(ULOG_ENVVAR));
      else if ((level < 0) || (level > 7))
	fprintf(stderr,
		"%s: WARNING -- %s must be value between 1 and 7 (default=%d; got %ld)\n",
		ident, ULOG_ENVVAR, default_log_level, level);
      else
