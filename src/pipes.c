 ...
 */
int pipe_open(struct Pipe_Handle *ph, const char *command_line) 
{
  ...
  if ( 0 == ph->child_pid ) {
    ...
    // Exec here
    ulog(LOG_INFO, "Child has set up its end. About to exec: %s -c %s", _PATH_BSHELL, ph->command_line);
    if ( execl(_PATH_BSHELL, _PATH_BSHELL, "-c", ph->command_line, NULL) == -1 ) {
      ulog(LOG_ERR, "Child was not able to exec: %s -c %s", _PATH_BSHELL, ph->command_line);
    ...
    close(ph->pipe_fds[0]);
    ulog(LOG_INFO, "Parent has set up its end. Child is pid=%d. Pipe fd=%d ", ph->child_pid, ph->pipe_fds[1]);
  }
 ...
 *    information. Returns 0 on success, non-zero on error.
 */
int pipe_open2(struct Pipe_Handle *ph, const char *command_line) 
{
  // Expect handle initialised and pipe not opened yet.
  assert(ph != NULL);
  assert(PIPES_CLOSED == ph->state);
...
int pipe_close(struct Pipe_Handle *ph)
{
  ...
  ulog_debug("waitpid(%d) returned %d. (exited=%d; signalled=%d; stopped=%d)", ph->child_pid, reaped_pid,
	     WIFEXITED(stat_loc), WIFSIGNALED(stat_loc), WIFSTOPPED(stat_loc));

  // Log what happened to child
  if ( reaped_pid == -1 ) {
    ulog(LOG_ERR, "Child process %d could not be reaped (%m)", ph->child_pid);
    perror("Error returned by waitpid");
  }
  else if ( WIFEXITED(stat_loc) ) {
    ulog(LOG_INFO, "Child process %d exited normally with status %d", ph->child_pid, WEXITSTATUS(stat_loc));
  }
  else if ( WIFSIGNALED(stat_loc) ) {
    ulog(LOG_INFO, "Child process %d terminated by signal %d", ph->child_pid, WTERMSIG(stat_loc));
  }
  else if ( WIFSTOPPED(stat_loc) ) {
    ulog(LOG_INFO, "Child process %d stopped by signal %d -- could be restarted", ph->child_pid, WSTOPSIG(stat_loc));
...
int pipe_reset(struct Pipe_Handle *ph)
{
  ...
  if ( rc != 0 ) {
    ulog(LOG_ERR, "Was not able to reset pipe because could not reopen: %s", ph->command_line);
    return rc;
  }
