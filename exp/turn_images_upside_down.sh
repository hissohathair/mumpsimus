...
ncat -l -k localhost 3128 -c "headers -c \"sed -El -e 's/^Cache-Control: .*/Cache-Control: no-cache/g'  -e 's/If-Modified-Since: .*/X-If-Modified-Since: none/g'\" | log | ncat $http_proxy_host $http_proxy_port | ULOG_LEVEL=7 body -c \"convert -flip -flop fd:0 fd:1\" | log -v"
